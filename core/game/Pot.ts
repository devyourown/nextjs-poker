...
export class Pot {
    ...
    private calculatePossibleMoney(): void {
        this.playerBetLog.forEach((betAmount, player) => {
            const canTakeMoney =
                betAmount * this.playerBetLog.size + this.foldAmount;
    ...
    splitMoney(winners: Player[], losers: Player[]): void {
        winners.sort(
            (a, b) =>
                a.getPossibleTakingAmountOfMoney() -
                b.getPossibleTakingAmountOfMoney()
        );
    ...
    private payBack(players: Player[], alreadyTaken: number): void {
        for (const player of players) {
            player.plusMoney(
                this.takeOutMoney(player.getBetSize() - alreadyTaken)
            );
        }
    }
