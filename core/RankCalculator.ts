export class RankingCalculator {
    static calculateCards(cards: Card[]): number {
        const numbersOfCards: number[] =
            RankingCalculator.convertToNumber(cards);
    ...
    private static getFullHouse(cardNumbers: number[]): number {
        ...
        const pairValue =
            RankingCalculator.getValueOfSamePair(leftWithoutTriple);
        if (pairValue > 0)
...
export function getRank(value: number): RankingString {
    ...
    const strings = Object.values(RankingString).filter((v) =>
        isNaN(Number(v))
    );
    for (let i = 0; i < 10; i++) {
